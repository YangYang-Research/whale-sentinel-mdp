...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function populateFormFromProfile(profile) {
        ...
        if (profile.ws_module_common_attack_detection) {
            ...
            document.getElementById("detect_unknown_attack").checked = c.detect_unknown_attack;
            document.getElementById("detect_insecure_redirect").checked = c.detect_insecure_redirect;
            document.getElementById("detect_insecure_file_upload").checked = c.detect_insecure_file_upload;
        }
        ...
        if (profile.secure_response_headers) {
            ...
            document.querySelectorAll(".secure-header").forEach(el => {
                ...
                el.checked = s.headers && s.headers[key] === value;
            });
    ...
    profileSelect.addEventListener("change", function () {
        ...
        try {
            if (profileData) {
                ...
                const pretty = JSON.stringify({ profile: originalAgentProfile }, null, 4);
                profileTextarea.value = pretty;
                populateFormFromProfile(originalAgentProfile.profile);
                errorMsg.classList.add("d-none");
